...
@switch ($event->event_type)
    @case(\App\Enums\EventType::Review)
        @empty($event->vote_type)
            <x-library-icon
                ...
                class="w-8"
                :lowerLeftIcon="!is_null($event->comment) ? LibraryIcon::Comment : null"
    ...
    @case(\App\Enums\EventType::Submit)
        <x-library-icon
            ...
            :lowerLeftIcon="!is_null($event->comment) ? LibraryIcon::Comment : null"
            lowerLeftColor="{{\App\Enums\VoteType::Comment->iconColor()}}"
            :lowerRightIcon="$event->initial_submit && !is_null($event->part->official_part) ? LibraryIcon::PartFix : null"
