...
class VectorMath
{
    public static function hasColinearPoints(array $points): bool
    {
        ...
        $angles[] = rad2deg(acos($p1->subtractVector($p2)->dotProduct($p3->subtractVector($p2)) / ($p1->subtractVector($p2)->length() * $p3->subtractVector($p2)->length())));

        if (count($points) == 3) {
            $angles[] = rad2deg(acos($p2->subtractVector($p3)->dotProduct($p1->subtractVector($p3)) / ($p2->subtractVector($p3)->length() * $p1->subtractVector($p3)->length())));
            $angles[] = rad2deg(acos($p3->subtractVector($p1)->dotProduct($p2->subtractVector($p1)) / ($p3->subtractVector($p1)->length() * $p2->subtractVector($p1)->length())));
        } else {
            $p4 = new Vector($points[3]);
            $angles[] = rad2deg(acos($p2->subtractVector($p3)->dotProduct($p4->subtractVector($p3)) / ($p2->subtractVector($p3)->length() * $p4->subtractVector($p3)->length())));
            $angles[] = rad2deg(acos($p3->subtractVector($p4)->dotProduct($p1->subtractVector($p4)) / ($p3->subtractVector($p4)->length() * $p1->subtractVector($p4)->length())));
            $angles[] = rad2deg(acos($p4->subtractVector($p1)->dotProduct($p2->subtractVector($p1)) / ($p4->subtractVector($p1)->length() * $p2->subtractVector($p1)->length())));
    ...
    public static function isCoplaner(array $points): bool
    {
        ...
        return $angle <= config('ldraw.check.coplaner_angle');
    }
}
